...
s = scipy.signal.chirp(t, f0=6, f1=1, t1=10)
plt.plot(t, s)
plt.show()
...
def incorrect_input_size(D, num_frames):
    ...
    # create STFT object
    stft = STFT(
        block_size, hop=hop, channels=D, transform=transform, num_frames=num_frames
    )
...
def no_overlap_with_filter(D, num_frames=1, fixed_memory=False, streaming=True):
    """
    D             - number of channels
    num_frames    - how many frames to process, None will process one frame at
                    a time
...
def call_all_stft_tests(num_frames=1, fixed_memory=False, streaming=True, overlap=True):
    ...
    print("no overlap, with filter, mono           : %0.0f dB" % (20 * np.log10(error)))

    error = no_overlap_with_filter(D, num_frames, fixed_memory, streaming)
    print("no overlap, with filter, multichannel   : %0.0f dB" % (20 * np.log10(error)))

    if overlap:
        error = with_half_overlap_no_filter(1, num_frames, fixed_memory, streaming)
        print(
            "half overlap, no filter, mono           : %0.0f dB"
            % (20 * np.log10(error))
        )

        error = with_arbitrary_overlap_synthesis_window(
            1, num_frames, fixed_memory, streaming, overlap=0.5
        )
        print(
            "half overlap, no filter, with synthesis windows, mono : %0.0f dB"
            % (20 * np.log10(error))
        )

        error = with_arbitrary_overlap_synthesis_window(
            D, num_frames, fixed_memory, streaming, overlap=0.5
        )
        print(
            "half overlap, no filter, with synthesis windows, multichannel : %0.0f dB"
            % (20 * np.log10(error))
        )

        error = with_arbitrary_overlap_synthesis_window(
            1, num_frames, fixed_memory, streaming, overlap=0.75
        )
        print(
            "3/4 overlap, no filter, with synthesis windows, mono : %0.0f dB"
            % (20 * np.log10(error))
        )

        error = with_arbitrary_overlap_synthesis_window(
            D, num_frames, fixed_memory, streaming, overlap=0.75
        )
        print(
            "3/4 overlap, no filter, with synthesis windows, multichannel : %0.0f dB"
            % (20 * np.log10(error))
        )

        error = with_arbitrary_overlap_synthesis_window(
            1, num_frames, fixed_memory, streaming, overlap=0.84
        )
        print(
            "84/100 overlap, no filter, with synthesis windows, mono : %0.0f dB"
            % (20 * np.log10(error))
        )

        error = with_arbitrary_overlap_synthesis_window(
            D, num_frames, fixed_memory, streaming, overlap=0.84
        )
        print(
            "84/100 overlap, no filter, with synthesis windows, multichannel : %0.0f dB"
            % (20 * np.log10(error))
        )

        error = with_arbitrary_overlap_synthesis_window(
            1, num_frames, fixed_memory, streaming, overlap=0.26
        )
        print(
            "26/100 overlap, no filter, with synthesis windows, mono : %0.0f dB"
            % (20 * np.log10(error))
        )

        error = with_arbitrary_overlap_synthesis_window(
            D, num_frames, fixed_memory, streaming, overlap=0.26
        )
        print(
            "26/100 overlap, no filter, with synthesis windows, multichannel : %0.0f dB"
            % (20 * np.log10(error))
        )

        error = with_arbitrary_overlap_synthesis_window(
            1, num_frames, fixed_memory, streaming, overlap=7 / 8
        )
        print(
            "7/8 overlap, no filter, with synthesis windows, mono : %0.0f dB"
            % (20 * np.log10(error))
        )

        error = with_arbitrary_overlap_synthesis_window(
            D, num_frames, fixed_memory, streaming, overlap=7 / 8
        )
        print(
            "7/8 overlap, no filter, with synthesis windows, multichannel : %0.0f dB"
            % (20 * np.log10(error))
        )

        error = with_arbitrary_overlap_synthesis_window(
            1, num_frames, fixed_memory, streaming, overlap=0.25
        )
        print(
            "1/4 overlap, no filter, with synthesis windows, mono : %0.0f dB"
            % (20 * np.log10(error))
        )

        error = with_arbitrary_overlap_synthesis_window(
            D, num_frames, fixed_memory, streaming, overlap=0.25
        )
        print(
            "1/4 overlap, no filter, with synthesis windows, multichannel : %0.0f dB"
            % (20 * np.log10(error))
        )

        error = with_half_overlap_no_filter(D, num_frames, fixed_memory, streaming)
        print(
            "half overlap, no filter, multichannel   : %0.0f dB"
            % (20 * np.log10(error))
        )

        error = with_half_overlap_with_filter(1, num_frames, fixed_memory, streaming)
        print(
            "half overlap, with filter, mono         : %0.0f dB"
            % (20 * np.log10(error))
        )

        error = with_half_overlap_with_filter(D, num_frames, fixed_memory, streaming)
        print(
            "half overlap, with filter, multichannel : %0.0f dB"
            % (20 * np.log10(error))
        )
...
class TestSTFT(TestCase):
    ...
    def test_no_overlap_no_filter_mono(self):
        error = no_overlap_no_filter(
            D=1, num_frames=1, fixed_memory=False, streaming=True
        )
        self.assertTrue(error < tol)
        error = no_overlap_no_filter(
            D=1, num_frames=50, fixed_memory=False, streaming=True
        )
        self.assertTrue(error < tol)
        error = no_overlap_no_filter(
            D=1, num_frames=1, fixed_memory=True, streaming=True
        )
        self.assertTrue(error < tol)
        error = no_overlap_no_filter(
            D=1, num_frames=50, fixed_memory=True, streaming=True
        )
        self.assertTrue(error < tol)
        error = no_overlap_no_filter(
            D=1, num_frames=1, fixed_memory=False, streaming=False
        )
        self.assertTrue(error < tol)
        error = no_overlap_no_filter(
            D=1, num_frames=50, fixed_memory=False, streaming=False
        )
        self.assertTrue(error < tol)
        error = no_overlap_no_filter(
            D=1, num_frames=1, fixed_memory=True, streaming=False
        )
        self.assertTrue(error < tol)
        error = no_overlap_no_filter(
            D=1, num_frames=50, fixed_memory=True, streaming=False
        )
        self.assertTrue(error < tol)

    def test_no_overlap_no_filter_multichannel(self):
        error = no_overlap_no_filter(
            D=D, num_frames=1, fixed_memory=False, streaming=True
        )
        self.assertTrue(error < tol)
        error = no_overlap_no_filter(
            D=D, num_frames=50, fixed_memory=False, streaming=True
        )
        self.assertTrue(error < tol)
        error = no_overlap_no_filter(
            D=D, num_frames=1, fixed_memory=True, streaming=True
        )
        self.assertTrue(error < tol)
        error = no_overlap_no_filter(
            D=D, num_frames=50, fixed_memory=True, streaming=True
        )
        self.assertTrue(error < tol)
        error = no_overlap_no_filter(
            D=D, num_frames=1, fixed_memory=False, streaming=False
        )
        self.assertTrue(error < tol)
        error = no_overlap_no_filter(
            D=D, num_frames=50, fixed_memory=False, streaming=False
        )
        self.assertTrue(error < tol)
        error = no_overlap_no_filter(
            D=D, num_frames=1, fixed_memory=True, streaming=False
        )
        self.assertTrue(error < tol)
        error = no_overlap_no_filter(
            D=D, num_frames=50, fixed_memory=True, streaming=False
        )
        self.assertTrue(error < tol)

    def test_no_overlap_with_filter_mono(self):
        error = no_overlap_with_filter(
            D=1, num_frames=1, fixed_memory=False, streaming=True
        )
        self.assertTrue(error < tol)
        error = no_overlap_with_filter(
    ...
    def test_with_half_overlap_no_filter_mono(self):
        error = with_half_overlap_no_filter(
            D=1, num_frames=1, fixed_memory=False, streaming=True
        )
        self.assertTrue(error < tol)
        error = with_half_overlap_no_filter(
    ...
    def test_with_half_overlap_with_filter_mono(self):
        error = with_half_overlap_with_filter(
            D=1, num_frames=1, fixed_memory=False, streaming=True
        )
        self.assertTrue(error < tol)
        error = with_half_overlap_with_filter(
    ...
    def test_with_arbitrary_overlap_synthesis_window_multichannel(self):
        ...
        for overlap in overlaps:
            error = with_arbitrary_overlap_synthesis_window(
                D, num_frames=1, fixed_memory=False, streaming=True, overlap=overlap
            )
            self.assertTrue(error < tol)
            error = with_arbitrary_overlap_synthesis_window(
                D, num_frames=50, fixed_memory=False, streaming=True, overlap=overlap
            )
            self.assertTrue(error < tol)
            error = with_arbitrary_overlap_synthesis_window(
                D, num_frames=1, fixed_memory=True, streaming=True, overlap=overlap
            )
            self.assertTrue(error < tol)
            error = with_arbitrary_overlap_synthesis_window(
                D, num_frames=50, fixed_memory=True, streaming=True, overlap=overlap
            )
            self.assertTrue(error < tol)
            error = with_arbitrary_overlap_synthesis_window(
                D, num_frames=50, fixed_memory=False, streaming=False, overlap=overlap
            )
            self.assertTrue(error < tol)
            error = with_arbitrary_overlap_synthesis_window(
                D, num_frames=50, fixed_memory=True, streaming=False, overlap=overlap
            )
            self.assertTrue(error < tol)
    ...
    def test_with_arbitrary_overlap_synthesis_window_mono(self):
        ...
        for overlap in overlaps:
            error = with_arbitrary_overlap_synthesis_window(
                1, num_frames=1, fixed_memory=False, streaming=True, overlap=overlap
            )
            self.assertTrue(error < tol)
            error = with_arbitrary_overlap_synthesis_window(
                1, num_frames=50, fixed_memory=False, streaming=True, overlap=overlap
            )
            self.assertTrue(error < tol)
            error = with_arbitrary_overlap_synthesis_window(
                1, num_frames=1, fixed_memory=True, streaming=True, overlap=overlap
            )
            self.assertTrue(error < tol)
            error = with_arbitrary_overlap_synthesis_window(
                1, num_frames=50, fixed_memory=True, streaming=True, overlap=overlap
            )
            self.assertTrue(error < tol)
            error = with_arbitrary_overlap_synthesis_window(
                1, num_frames=50, fixed_memory=False, streaming=False, overlap=overlap
            )
            self.assertTrue(error < tol)
            error = with_arbitrary_overlap_synthesis_window(
...
if __name__ == "__main__":
    ...
    call_all_stft_tests(num_frames=num_frames, fixed_memory=True, streaming=True)
    ...
    call_all_stft_tests(
        num_frames=1, fixed_memory=False, streaming=False, overlap=False
    )
    ...
    call_all_stft_tests(num_frames=1, fixed_memory=True, streaming=False, overlap=False)
